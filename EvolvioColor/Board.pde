...
class Board{
  ...
  public void drawBoard(float scaleUp, float camZoom, int mX, int mY, float tX, float tY){
    Profiler.measure("Board.drawBoard");
    ...
    // Inverse camera zoom
    float cm = 1/camZoom * 52;
    
    // userControl being true implies the screen can be rotated
    if(userControl)
      cm *= 1.41421356237; // Multiply by sqrt2 so corner tiles aren't cut off when the board is rotated
    
    // Setup boundaries for the board renderer
    int xStart = tX-cm < 0 ? 0 : (int)(tX-cm), xEnd = (tX+cm > boardWidth  ? boardWidth  : (int)(tX+cm)+1);
    int yStart = tY-cm < 0 ? 0 : (int)(tY-cm), yEnd = (tY+cm > boardHeight ? boardHeight : (int)(tY+cm)+1);
    int iterateGroup = frameCount % 4;

    for(int x = 0; x < boardWidth; x++){
      for(int y = 0; y < boardHeight; y++){    
         // Update every fourth tile each frame to reduce total strain
        if(iterateGroup == (x + y) % 4)
          tiles[x][y].iterate();
        
        // Draw tiles that are visible on-screen
        if(x >= xStart && x <= xEnd && y >= yStart && y <= yEnd)
          tiles[x][y].drawTile(scaleUp, (camZoom > 2.0 && camZoom < 95.0 && mX == x && mY == y));
      }
    }
    
    // Don't draw soft bodies if the user is zoomed out too far
    // Not only are soft bodies difficult to see at this zoom level, they add unnecessary
    // overhead for the renderer, especially when all of them are on screen at once
    if(camZoom < 1.5) {
      Profiler.stop();
      return;
    }
    ...
    for(int i = 0; i < creatures.size(); i++){
      creatures.get(i).drawSoftBody(scaleUp, camZoom, true, tX, tY, cm);
    }
  ...
  public void drawBlankBoard(float scaleUp){
    Profiler.measure("Board.drawBlankBoard");
  ...
  public void drawUI(float scaleUp,double timeStep, int x1, int y1, int x2, int y2, PFont font){
    Profiler.measure("Board.drawUI");
    
    // For every other frame, draw the UI image cached from the previous frame 
    // Most users won't notice the UI being updated every other frame and this optimization is ~1.6 times faster
    if(frameCount % 3 < 2 && UICache != null) {
      Profiler.measure("Board.drawUI.image");
      image(UICache, x1, y1);
      Profiler.stop();
      Profiler.stop();
      return;
    ...
    Profiler.measure("Board.drawUI.displaySeasons");
    fill(0,0,0);
    noStroke();
    ...
    if(selectedCreature == null){
      Profiler.measure("Board.drawUI.creatureList");
      ...
      for(int i = 0; i < creatures.size(); i++){
        ...
        // A switch-case statement would make more sense here, but it's not necessary
      ...
      Profiler.measure("Board.drawUI.selectedCreature");
    ...
    UICache = get(x1, y1, x2-x1, y2-y1);
    
    Profiler.stop();
  }
  void drawPopulationGraph(float x1, float x2, float y1, float y2){
    Profiler.measure("Board.drawPopulationGraph");
  ...
  public void iterate(double timeStep){
    Profiler.measure("Board.iterate");
    ...
    for(int i = 0; i < creatures.size(); i++){
      ...
      if(userControl){
        ...
        if(me == selectedCreature){
          // All of this is better suited for a switch-case statement
          if(keyPressed){
             if (key == CODED) {
  ...
  public void finishIterate(double timeStep){
    Profiler.measure("Board.finishIterate");
  ...
  private void drawCreature(Creature c, float x, float y, float scale, float scaleUp){
    ...
    translate(x,y);
    c.drawSoftBody(scaleIconUp, 40.0/scale,false, 0.0, 0.0, 0.0);
    popMatrix();
  }
  ...
  private void fileSave() {
    Profiler.measure("Board.fileSave");
    for(int i = 0; i < 4; i++){

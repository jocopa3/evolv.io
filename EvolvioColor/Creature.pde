class Creature extends SoftBody{
  ...
  Axon[][][] tbrain, double[][] tneurons, int tgen, double tmouthHue){
    ...
    Profiler.measure("Creature.Constructor");
    if(tbrain == null){
  ...
  public void drawBrain(PFont font, float scaleUp, int mX, int mY){
    Profiler.measure("Creature.drawBrain");
  ...
  public void useBrain(double timeStep, boolean useOutput){
    Profiler.measure("Creature.useBrain");
    for(int i = 0; i < 9; i++){
  ...
  public void drawSoftBody(float scaleUp, float camZoom, boolean showVision, float tX, float tY, float cm){
    if(showVision && (px < tX - cm || px > tX + cm) && (py < tY - cm || py > tY + cm))
      return;
      
    Profiler.measure("Creature.drawSoftBody");
    ellipseMode(RADIUS);
    double radius = getRadius();
    if(showVision && cm < 25.0){
      Profiler.measure("Creature.drawSoftBody.drawVision");
    ...
    Profiler.measure("Creature.drawSoftBody.drawBody");
    strokeWeight(board.CREATURE_STROKE_WEIGHT);
    stroke(0,0,1);
    fill(0,0,1);
    ...
    Profiler.stop();
    if(showVision && cm < 8.0){
      Profiler.measure("Creature.drawSoftBody.drawName");
      fill(0,0,1);
  ...
  public void metabolize(double timeStep){
    Profiler.measure("Creature.metabolize");
  ...
  public void fight(double amount, double timeStep){
    Profiler.measure("Creature.fight");
  ...
  public void reproduce(double babySize, double timeStep){
    Profiler.measure("Creature.reproduce");
    if(colliders == null){
      collide(timeStep);
    }
    int highestGen = 0;
